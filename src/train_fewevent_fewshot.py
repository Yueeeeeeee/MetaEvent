...
from dataloader_fewevent import get_fewevent_loaders
...
def evaluation(args, model, scaler, inner_loop_optimizer, eval_iter):
    ...
    with torch.autocast(device_type=args.device, dtype=torch.float16):
        for task in range(len(support_set)):
            ...
            f1_macro.append(f1_score(gts, preds, average='macro'))
    
    return np.mean(f1_micro), np.mean(f1_macro)
...
def train_fewshot(args):
    ...
    train_loader, val_loader, test_loader, vocab = get_fewevent_loaders(args, args.N, args.K, args.max_len, 
                                                    tokenizer, prompt_template=args.template, train_size=args.num_task)
    ...
    test_f1_micro, test_f1_macro = evaluation(args, model, scaler, inner_loop_optimizer, test_iter)
    ...
    with open(os.path.join(export_root, 'test_metrics.json'), 'w') as f:
        json.dump([test_f1_micro, test_f1_macro], f) 


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--seed', default=42, type=int, help='random seed for the experiments')
    parser.add_argument('--fewevent_data_dir', default='../data/fewevent', type=str, help='data path for FewEvent dataset')
    parser.add_argument('--fewevent_train_file', default='fewshot_train.json', type=str, help='train file for MAVEN dataset')
    parser.add_argument('--fewevent_dev_file', default='fewshot_dev.json', type=str, help='dev file for MAVEN dataset')
    parser.add_argument('--fewevent_test_file', default='fewshot_test.json', type=str, help='test file for MAVEN dataset')
